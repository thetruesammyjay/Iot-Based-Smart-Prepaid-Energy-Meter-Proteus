...
│
├── README.md                          # Project overview, setup guide, and validation instructions
├── FILE-STRUCTURE.md                  # This document - annotated project directory layout
├── GET-STARTED.md                     # Pre-coding setup guide: toolchain, libraries, Wokwi, IoT platform, and verification
│
├── wokwi/                             # Browser-based ESP32 firmware validation files
│   ├── diagram.json                   # Wokwi circuit definition and peripheral wiring
│   ├── wokwi.toml                     # Wokwi project configuration
│   ├── mock-data/                     # Mock serial frames and validation inputs
│   └── README.md                      # Validation notes and usage instructions
│
...
│   ├── circuit-description.md         # Component-level explanation of the prototype circuit
│   ├── component-list.md              # Bill of Materials (BOM) with part numbers and sourcing notes
│   ├── validation-guide.md            # Step-by-step instructions for firmware validation and bench testing
...
│   ├── circuit-schematic.png          # Full circuit schematic export for the physical prototype
...
│   ├── validation-captures/           # Captured screenshots from Wokwi and bench validation
...
### `wokwi/`
Contains the browser-based firmware validation project for the ESP32. The `diagram.json` file defines the virtual wiring and component layout, while `mock-data/` stores serial frames and test inputs used during validation. Any changes to the firmware source code require recompilation in the Arduino IDE (with the Espressif Arduino Core) to regenerate the updated logic before re-testing.
...
### `docs/`
Contains all supplementary technical documentation. The `validation-guide.md` is particularly important for first-time users setting up the Wokwi validation flow and bench testing steps. The `billing-algorithm.md` provides the formal mathematical basis for energy metering and credit calculations.

### `schematics/`
Exported static images of all circuit diagrams and system flowcharts. These are suitable for inclusion in the project report or presentation without requiring the validation environment to be opened.

### `assets/`
Contains validation captures demonstrating key system states and component datasheets used as hardware references during design.
...
| File | Depends On |
|---|---|
| `wokwi/diagram.json` | `firmware/SmartEnergyMeter.ino`, `firmware/src/` modules, `wokwi/mock-data/` |
